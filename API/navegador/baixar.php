...
require("https://www.baixar-videos.net/API/GoogleAnalytics.php");
require("../get_video_id.php");
...
$json = json_decode(file_get_contents("https://www.baixar-videos.net/API/privadas/get_video_info_download.php?u=".$ID_VIDEO));
...
if ((strpos($f, 'mp4') !== false)) {
    echo '<a href="'.$e[$i]->{'url'}.'" download target="_blank" class="btn btn-default btn-lg baixar_link" style="text-decoration: none;color: #333;display: inline-block;padding: 6px 12px;border: 1px solid transparent;border-radius: 4px;border-color: rgb(15, 158, 222);"> <span lang="BOTAO_BAIXAR_VIDEO">Baixar</span> - '.$f_s.'</a> <span class="label label-primary">'.$e[$i]->{'tamanho'}.'</span><br><br>';
}
}
}else{
    echo '
    <br><br>
    <div class="row" style="background:url(//www.baixar-videos.net/img/Loading_icon.gif) top center no-repeat;background-size: 90px;"><div class="container"><center><iframe id="ytubeframe" src="https://odg.youtube6download.top/cnvx.php?id='.$ID_VIDEO.'" width="100%" scrolling="no" style="border:none;"></iframe></center></div></div>
    <br>
    <div class="row" style="background:url(//www.baixar-videos.net/img/Loading_icon.gif) top center no-repeat;background-size: 90px;"><center><iframe id="ytubeframe" src="https://www.yt-download.org/api-console/video/'.$ID_VIDEO.'" width="100%" scrolling="no" style="border:none;"></iframe></center></div>
    <br><br>';
}
